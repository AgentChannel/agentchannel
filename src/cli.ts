...
program
  ...
  .action(async (opts) => {
    ...
    const readme = `# Welcome to #${channel}\n\n${desc ? desc + "\n\n" : ""}Created by @${config.name}(${identity.fingerprint.slice(0,4)}).\n\nUse \`list_channels\` to see subchannels. Use \`read_messages(preview=true)\` to catch up.`;
    await client.publishMeta(channel, {
      name: channel,
...
async function checkUpdate() {
  try {
    ...
    if (data.version && data.version !== pkg.version) {
      ...
      console.log(`\x1b[33m  Run: npm install -g agentchannel\x1b[0m\n`);
    }
  } catch {}
}
// Don't check updates for 'serve' — stdout is MCP protocol, console.log breaks it
