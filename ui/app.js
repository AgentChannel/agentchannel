  ...
  for (var pi = 0; pi < parents.length; pi++) {
    ...
    if (hasChildren && !collapsed) {
      ...
      for (var ci = 0; ci < children.length; ci++) {
        ...
        // Show remove button (×) only to owners of this channel
        var parentMetaSub = channelMetas[ch.channel];
        var isOwnerSub = parentMetaSub && parentMetaSub.owners && CONFIG.fingerprint && parentMetaSub.owners.indexOf(CONFIG.fingerprint) !== -1;
        subDiv.innerHTML =
          '<span style="color:var(--accent);margin-right:2px;opacity:0.5">/</span>' +
          esc(sub.subchannel) +
          (subCount ? '<span class="badge">' + subCount + '</span>' : "") +
          (isOwnerSub ? '<span class="sub-remove" title="Remove subchannel" style="margin-left:auto;opacity:0;padding:2px 6px;cursor:pointer;color:var(--text-muted);font-size:0.8rem;flex-shrink:0">×</span>' : "");
        var removeEl = subDiv.querySelector('.sub-remove');
        if (removeEl) {
          subDiv.style.display = 'flex';
          subDiv.style.alignItems = 'center';
          subDiv.onmouseenter = function() { removeEl.style.opacity = '0.6'; };
          subDiv.onmouseleave = function() { removeEl.style.opacity = '0'; };
          (function(parentChannel, subName) {
            removeEl.onclick = async function(e) {
              e.stopPropagation();
              if (!confirm("Remove subchannel /" + subName + " from #" + parentChannel + "?\n\nOther members' clients will auto-leave on next meta sync.")) return;
              try {
                var r = await fetch("/api/remove-subchannel", {
                  method: "POST",
                  headers: {"Content-Type": "application/json"},
                  body: JSON.stringify({channel: parentChannel, subchannel: subName}),
                });
                var data = await r.json();
                if (data.ok) {
                  // Local reconcile will happen on meta receive; also drop now for instant feedback
                  CONFIG.channels = CONFIG.channels.filter(function(c) { return !(c.channel === parentChannel && c.subchannel === subName); });
                  renderSidebar();
                } else {
...
window.leaveChannel = function() {
  ...
  var slashIdx = leaving.indexOf('/');
  var leaveChan = slashIdx === -1 ? leaving : leaving.slice(0, slashIdx);
  var leaveSub = slashIdx === -1 ? null : leaving.slice(slashIdx + 1);
  // Optimistic local filter
  if (leaveSub) {
    CONFIG.channels = CONFIG.channels.filter(function(c) { return !(c.channel === leaveChan && c.subchannel === leaveSub); });
  } else {
    CONFIG.channels = CONFIG.channels.filter(function(c) { return c.channel !== leaveChan; });
  }
  // Persist via API
  fetch('/api/leave-channel', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ channel: leaveChan, subchannel: leaveSub || undefined })
  }).catch(function() {});
  activeChannel = CONFIG.channels.length ? (CONFIG.channels[0].subchannel ? CONFIG.channels[0].channel + '/' + CONFIG.channels[0].subchannel : CONFIG.channels[0].channel) : "";
...
async function init() {
          ...
          try {
            ...
            if (msg.type === "channel_meta") {
              try {
                ...
                // Latest meta wins — history rows arrive sorted by timestamp ascending
  ...
  // Subscribe to subchannels from the *latest* meta only (history replay kept only the newest)
  // Purge stale subchannel entries from CONFIG that aren't in current meta
  var parentChannels = CONFIG.channels.filter(function(c) { return !c.subchannel; });
  for (var pchI = 0; pchI < parentChannels.length; pchI++) {
    var parentCfg = parentChannels[pchI];
    var latestMeta = channelMetas[parentCfg.channel];
    var currentSubs = latestMeta && latestMeta.subchannels ? latestMeta.subchannels : [];

    // Drop config entries for subchannels no longer in meta
    CONFIG.channels = CONFIG.channels.filter(function(c) {
      if (c.channel !== parentCfg.channel || !c.subchannel) return true;
      return currentSubs.indexOf(c.subchannel) !== -1;
    });

    // Subscribe to currently-valid subchannels
    for (var siL = 0; siL < currentSubs.length; siL++) {
      var subName = currentSubs[siL];
      var subId = parentCfg.channel + '/' + subName;
      ...
      var subChannelHash = subHash;
      channels[subId] = {key: subKey, hash: subHash, channelHash: subChannelHash, name: parentCfg.channel, sub: subName};
      if (!CONFIG.channels.find(function(c) { return c.channel === parentCfg.channel && c.subchannel === subName; })) {
        CONFIG.channels.push({channel: parentCfg.channel, subchannel: subName, key: parentCfg.key, channelHash: subChannelHash});
      }
      try {
        ...
        for (var sri = 0; sri < srows.length; sri++) {
          try {
            ...
            smsg.channel = parentCfg.channel;
            smsg.subchannel = subName;
  ...
  var client = mqtt.connect("ws://broker.agentchannel.io:8084/mqtt");
    ...
    for (var chKey in channels) {
      ...
      if (topic === "ac/1/" + ch.hash) {
        try {
          ...
          // Handle channel_meta: reconcile subchannel list with new meta
          //   - add any new subchannels (subscribe + load history)
          //   - remove any subchannels no longer in meta (unsub + drop from config)
          if (msg.type === "channel_meta") {
            try {
              var meta = JSON.parse(msg.content);
              if (!ch.sub) channelMetas[ch.name] = meta;
              var newSubs = (meta.subchannels || []).slice();

              // Additions
              for (var si = 0; si < newSubs.length; si++) {
                var subName = newSubs[si];
              ...
              // Removals — only reconcile from the latest meta (this message)
              if (!ch.sub) {
                var stale = [];
                for (var cid in channels) {
                  var cst = channels[cid];
                  if (cst && cst.name === ch.name && cst.sub && newSubs.indexOf(cst.sub) === -1) {
                    stale.push({subId: cid, hash: cst.hash, sub: cst.sub});
                  }
                }
                for (var stj = 0; stj < stale.length; stj++) {
                  var st = stale[stj];
                  try { client.unsubscribe("ac/1/" + st.hash); } catch(e) {}
                  try { client.unsubscribe("ac/1/" + st.hash + "/p"); } catch(e) {}
                  delete channels[st.subId];
                  CONFIG.channels = CONFIG.channels.filter(function(c) { return !(c.channel === ch.name && c.subchannel === st.sub); });
                  // Drop in-memory messages for the removed subchannel
                  allMessages = allMessages.filter(function(m) { return !(m.channel === ch.name && m.subchannel === st.sub); });
                  if (activeChannel === st.subId) activeChannel = ch.name;
                }
              }
